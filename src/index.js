...
class TasksList extends React.Component {
    ...
    render() {
        ...
        let done = tasksDone.length;
        let undone = tasks.length - tasksDone.length;
        ...
        return (
            <ul>
                ...
                <div className="">Всего {all} пунктов Сделано {done} пунктов Осталось {undone} пунктов</div>
            </ul>
        );
    }
}
...
class NewTaskForm extends React.Component { // Компонент доска
    constructor(props) {
        super(props);
        this.inputRef = React.createRef(); // (?) Так можно сделать автофокус?
        this.state = {
            value: '',
        };
    ...
    // (?) Так можно сделать автофокус?


    render() {
        return (
            <form onSubmit = {this.handleSubmit} className="new-task">
                <input ref={this.inputRef} autoFocus type="text" onChange = {this.handleChange} placeholder="Add new item"
